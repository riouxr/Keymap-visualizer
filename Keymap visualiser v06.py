...
def is_relevant_keymap(km, editor):
    ...
    if editor == 'VIEW_3D':
        return (
            any(n in name for n in (
                "3D View", "Object Mode", "Mesh", "Sculpt", "Pose", "Armature",
                "Weight Paint", "Vertex Paint", "Screen", "Window", "User Interface",
                "PME", "Tool:"))
        )
    if editor == 'UV':
        return "UV" in name or "Image" in name or "Screen" in name or "Window" in name
    return False
